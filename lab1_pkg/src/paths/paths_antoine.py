...
class LinearPath(MotionPath):
    ...
    def target_position(self, time):
        ...
        return pg.pos(time, self.total_time) * (self.target_pos - self.current_pos)
    ...
    def target_velocity(self, time):
        ...
        return pg.vel(time, self.total_time) * (self.target_pos - self.current_pos) / self.total_time
    ...
    def target_acceleration(self, time):
        ...
        """
        # return vec(0,0,0)
        return pg.acc(time, self.total_time) * (self.target_pos - self.current_pos) / self.total_time**2
...
class MultiplePaths(MotionPath):
    ...
    def target_position(self, time):
        ...
        return pg.pos(t, self.t_segment) * (pos_end - pos_start)
    ...
    def target_velocity(self, time):
        ...
        t = time - segment * t_segment
        return pg.vel(t, self.t_segment) * (pos_end - pos_start) / t_segment
    ...
    def target_acceleration(self, time):
        ...
        if segment < len(self.pos) - 1:
            ...
            pos_end = self.pos[1]
        t = time - segment * t_segment
        return pg.acc(time, self.t_segment) * (pos_end - pos_start) / t_segment**2
